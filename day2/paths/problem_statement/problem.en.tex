...
A {\em path} in the graph is defined as an ordered list of $2$ or more vertices, such that
there are edges between consecutive vertices in the list. In this task we are only interested in {\em simple paths} in which no vertex occurs more than once. Note that the list is ordered; for example, ``\texttt{5-6-7}'', ``\texttt{5-7-6}'' and ``\texttt{7-6-5}'' are all treated as different paths.
